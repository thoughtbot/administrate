...
describe Administrate::Field::Number do
  ...
  describe "#to_partial_path" do
    it "returns a partial based on the page being rendered" do
      page = :show
      number = double
      field = Administrate::Field::Number.new(Class.new.new,:price, number, page)
  ...
  describe "#to_s" do
    it "defaults to displaying no decimal points" do
      int = Administrate::Field::Number.new(Class.new.new,:quantity, 3, :show)
      float = Administrate::Field::Number.new(Class.new.new,:quantity, 3.1415926, :show)
    ...
    context "when data is nil" do
      it "returns a dash" do
        number = Administrate::Field::Number.new(Class.new.new,:number, nil, :page)
    ...
    def number_with_options(num, options)
      Administrate::Field::Number.new(Class.new.new,:number, num, :page, options)
    end
  end
end
